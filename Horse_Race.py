...
ns, A_lis, B_lis = generate_input(seed=2, n_min=4, n_max=7)
...
for f in function_wo_parameter.keys():
    ...
    write_csv(ns[0:11], mean, std, f'horse_race/{function_wo_parameter[f]}_small.csv')

# Benchmarking with parameter

for f in function_w_parameter.keys():
    mean, std = benchmark_runtime(f, (A_lis, B_lis), 3, 256)
    write_csv(ns, mean, std, f'horse_race/{function_w_parameter[f]}_small.csv')

# Transpose before benchmarking transposed
for matrix in B_lis:
    transpose(matrix)

mean, std = benchmark_runtime(elementary_multiplication_transposed, (A_lis, B_lis), 3)
write_csv(ns, mean, std, f'horse_race/Elementary_Transposed_small.csv')